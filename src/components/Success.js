  ...
  return (
    ...
    <div  className={`${type==="low"? "w-[100%]":"w-[50%]"} flex flex-col justify-center items-center`}>
      ...
      <div className='w-full flex justify-center items-center bg-white flex-col shadow-md p-6 rounded-b-lg'>
        <p className={`text-gray-500 ${type==="low"? "text-sm": type==="medium"? "text-base" : type==="high"?"text-lg":"text-sm"} text-center  tracking-wide`}>Congratulations, {title}</p>                
        <p className={`text-gray-500 ${type==="low"? "text-sm": type==="medium"? "text-base" : type==="high"?"text-lg":"text-sm"} text-center mt-3  tracking-wide`}>has been successfully {action}</p>   
